...
<script lang="ts">
  ...
  import { getcommentListByPage, getInfo,deleteByIds } from '/@/api/comment/comment.ts';

  export default defineComponent({
    name: 'comment',
    components: { BasicTable, TableAction, PageWrapper ,commentModal },
    setup() {
      const [registerModal, { openModal,setModalProps }] = useModal();
      const [registerTable, { reload ,setLoading}] = useTable({
        title: '列表',
        api: getcommentListByPage,
        columns,
        formConfig: {
          labelWidth: 120,
